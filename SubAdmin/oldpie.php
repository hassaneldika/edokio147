		...
		<div class="left-content">
			<div class="mother-grid-inner">
				...
				if($_SESSION["userid"]==true) {
					...
					$conx = new Service;
					$conx->connect();
					$stmt = $conx->dbh->prepare("SELECT S.SessionName, COUNT(*) AS TotalRows FROM session S JOIN examgrades E ON E.sessionid = S.SessionId AND E.admin = :admin  GROUP BY S.SessionName");
					$stmt->bindParam(':admin',$admin);
					$stmt->execute();
					$result = $stmt->fetchAll();
                      // var_dump($result);
                       
					$stmt = $conx->dbh->prepare("SELECT * FROM examgrades Where admin=:admin");
					$stmt->bindParam(':admin',$admin);
					$stmt->execute();
					...
					<div class="agile-grids">
						<div class="agile-tables">
							...
							<h2>Modules Traffic: (<?php echo $Counter;?>) Users</h2>
