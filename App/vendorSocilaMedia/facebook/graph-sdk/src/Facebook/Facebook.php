...
class Facebook
{
     ...
     *
     * @param array $configfacebook
     *
     * @throws FacebookSDKException
     */
    public function __construct(array $configfacebook = [])
    {
        $configfacebook = array_merge([
        ...
        if (!$configfacebook['app_id']) {
            throw new FacebookSDKException('Required "app_id" key not supplied in configfacebook and could not find fallback environment variable "' . static::APP_ID_ENV_NAME . '"');
        }
        if (!$configfacebook['app_secret']) {
            throw new FacebookSDKException('Required "app_secret" key not supplied in configfacebook and could not find fallback environment variable "' . static::APP_SECRET_ENV_NAME . '"');
        }

        $this->app = new FacebookApp($configfacebook['app_id'], $configfacebook['app_secret']);
        $this->client = new FacebookClient(
            HttpClientsFactory::createHttpClient($configfacebook['http_client_handler']),
            $configfacebook['enable_beta_mode']
        );
        $this->pseudoRandomStringGenerator = PseudoRandomStringGeneratorFactory::createPseudoRandomStringGenerator(
            $configfacebook['pseudo_random_string_generator']
        );
        $this->setUrlDetectionHandler($configfacebook['url_detection_handler'] ?: new FacebookUrlDetectionHandler());
        $this->persistentDataHandler = PersistentDataFactory::createPersistentDataHandler(
            $configfacebook['persistent_data_handler']
        );

        if (isset($configfacebook['default_access_token'])) {
            $this->setDefaultAccessToken($configfacebook['default_access_token']);
        }

        // @todo v6: Throw an InvalidArgumentException if "default_graph_version" is not set
        $this->defaultGraphVersion = $configfacebook['default_graph_version'];
    }
